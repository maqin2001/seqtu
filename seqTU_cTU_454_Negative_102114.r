...
#file_454TU <- "/home/wc102/project/tu/data/454.Treatment.CoveredReads.Length.ChangeRatio"
...
num=1
for(i in 1:(nrow(AllReverseGenes))){
    	if ((AllReverseGenesExpressionMean[i] >= MinimumMeanExpressionValue)&(AllReverseGenesExpressionMedian[i] >= MinimumMedianExpressionValue)&(AllReverseGenesGapsProportion[i] < MaximumGapProportionValue)&(AllReverseGenesGapsLongest[i] < 50)){
...
for(i in 1:length(Temp454PositiveIndexInTest)){
	cat(i,"\t",Temp454PositiveIndexInTest[i],"\n",sep="")
}
sink()
