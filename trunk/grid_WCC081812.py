...
if not is_win32:
       svmtrain_exe = "/srv2/www/html/SeqTU/bin/libsvm-2.89/svm-train"
       gnuplot_exe = "/usr/bin/gnuplot"
else:
       # example for windows
       svmtrain_exe = r"..\windows\svm-train.exe"
       gnuplot_exe = r"c:\tmp\gnuplTheit\bin\pgnuplot.exe"

# global parameters and their default values

fold = 5
c_begin, c_end, c_step = 0.1,  0.9, 0.02
    ...
    def run(self):
        while True:
            ...
            if cexp is WorkerStopToken:
                ...
                break
            try:
                rate = self.run_one(cexp,2.0**gexp)
...
class SSHWorker(Worker):
    def __init__(self,name,job_queue,result_queue,host):
        ...
        self.cwd = os.getcwd()
    def run_one(self,c,g):
        cmdline = 'ssh -x %s "cd %s; %s -n %s -g %s -v %s %s %s"' % \
          (self.host,self.cwd,
           svmtrain_exe,c,g,fold,pass_through_string,dataset_pathname)
        result = Popen(cmdline,shell=True,stdout=PIPE).stdout
        for line in result.readlines():
